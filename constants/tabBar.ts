...
// A wide island that still leaves a cream margin either side — each regular
// icon (42px circle) gets a roomy slot; the Scan button (50px) gets a wider
// one so it doesn't crowd its neighbors. Total is centered on screen at
// render time (see _layout.tsx), not pinned to fixed side margins.
export const TAB_BAR_ITEM_WIDTH = 62;
export const TAB_BAR_SCAN_ITEM_WIDTH = 74;
